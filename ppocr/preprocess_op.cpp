...
namespace PaddleOCR {
    ...
    void ResizeImgType0::Run(const cv::Mat &img, cv::Mat &resize_img, int limit_side_len,
                             float &ratio_h, float &ratio_w) {
        int w = img.cols;
        int h = img.rows;
        float ratio = 1.f;
        std::string limit_type = "";
        if (w > limit_side_len || h > limit_side_len) {
            limit_type = "max";
        } else {
            limit_type = "min";
        }
        ...
        cv::copyMakeBorder(resize_img, resize_img, 0, limit_side_len - resize_h, 0, limit_side_len - resize_w, cv::BORDER_CONSTANT, cv::Scalar(128,128,128));
